...
function solver(bridgesPuzzle, depth = 3) {
  ...
  const { solved, steps } = solveIterative$1(puzzle, depth);
  return { solved, solution: puzzle, steps };
}
/**
 * Loops through solverSteps until the puzzle is solved.
 */
function solveIterative$1(puzzle, depth) {
  let solved = false;
  const steps = [];
  let oldPuzzle = "";
  let newPuzzle = JSON.stringify(puzzle);
  ...
  while (oldPuzzle !== newPuzzle) {
    oldPuzzle = newPuzzle;
    solved = solverStep$1(puzzle, depth).solved;
    newPuzzle = JSON.stringify(puzzle);
    // save a copy of the current puzzle
    steps.push(JSON.parse(newPuzzle));
    // exit loop if the puzzle is solved
 ...
 * multiple times until the puzzle is solved.
 */
function solverStep$1(puzzle, depth) {
  if (depth < 0) {
    return { solved: false };
  }
  const graph = createGraph(puzzle);
...
exports.solveIterative = solveIterative$1;
exports.solver = solver;
exports.solverStep = solverStep$1;
